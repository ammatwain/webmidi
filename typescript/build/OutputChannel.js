 ...
 * @since 3.0.0
 */
class OutputChannel extends EventEmitter_1.EventEmitter {
    /**
    ...
    sendKeyAftertouch(target, pressure, options) {
        ...
        if (!options.rawValue)
            pressure = Utilities_1.Utilities.fromFloatTo7Bit(pressure);
        // Plot total offset
        const offset = WebMidi_1.WebMidi.octaveOffset + this.output.octaveOffset + this.octaveOffset;
        // Make sure we are dealing with an array
        if (!Array.isArray(target))
            target = [target];
        Utilities_1.Utilities.buildNoteArray(target).forEach(n => {
            this.send([
                (Enumerations_1.Enumerations.CHANNEL_MESSAGES.keyaftertouch << 4) + (this.number - 1),
                n.getOffsetNumber(offset),
                pressure
            ], { time: Utilities_1.Utilities.toTimestamp(options.time) });
        });
        return this;
    }
    /**
    ...
    sendControlChange(controller, value, options) {
        if (typeof controller === "string") {
            controller = Utilities_1.Utilities.getCcNumberByName(controller);
        }
        ...
        value.forEach((item, index) => {
            this.send([
                (Enumerations_1.Enumerations.CHANNEL_MESSAGES.controlchange << 4) + (this.number - 1),
                controller + (index * 32),
                value[index]
            ], { time: Utilities_1.Utilities.toTimestamp(options.time) });
        });
   ...
   */
    ...
    sendRpnDecrement(parameter, options) {
        if (!Array.isArray(parameter))
            parameter = Enumerations_1.Enumerations.REGISTERED_PARAMETERS[parameter];
        if (WebMidi_1.WebMidi.validation) {
            if (parameter === undefined) {
                throw new TypeError("The specified registered parameter is invalid.");
            }
            let valid = false;
            Object.getOwnPropertyNames(Enumerations_1.Enumerations.REGISTERED_PARAMETERS).forEach(p => {
                if (Enumerations_1.Enumerations.REGISTERED_PARAMETERS[p][0] === parameter[0] &&
                    Enumerations_1.Enumerations.REGISTERED_PARAMETERS[p][1] === parameter[1]) {
                    valid = true;
                }
            });
            if (!valid)
        ...
        for (let note of notes) {
            if (parseInt(note.duration) > 0) {
                const noteOffOptions = {
                    time: (Utilities_1.Utilities.toTimestamp(options.time) || WebMidi_1.WebMidi.time) + parseInt(note.duration),
                ...
                const noteOffOptions = {
                    time: (Utilities_1.Utilities.toTimestamp(options.time) || WebMidi_1.WebMidi.time) + parseInt(options.duration),
    ...
    sendNoteOff(note, options) {
        ...
        const offset = WebMidi_1.WebMidi.octaveOffset + this.output.octaveOffset + this.octaveOffset;
        Utilities_1.Utilities.buildNoteArray(note, { rawRelease: parseInt(nVelocity) }).forEach(n => {
            this.send([
                (Enumerations_1.Enumerations.CHANNEL_MESSAGES.noteoff << 4) + (this.number - 1),
                n.getOffsetNumber(offset),
                n.rawRelease,
            ], { time: Utilities_1.Utilities.toTimestamp(options.time) });
        });
    ...
    sendNoteOn(note, options) {
        ...
        // Plot total octave offset
        const offset = WebMidi_1.WebMidi.octaveOffset + this.output.octaveOffset + this.octaveOffset;
        Utilities_1.Utilities.buildNoteArray(note, { rawAttack: nVelocity }).forEach(n => {
            this.send([
                (Enumerations_1.Enumerations.CHANNEL_MESSAGES.noteon << 4) + (this.number - 1),
                n.getOffsetNumber(offset),
                n.rawAttack
            ], { time: Utilities_1.Utilities.toTimestamp(options.time) });
        });
    ...
    sendChannelMode(command, value, options) {
        ...
        if (typeof command === "string")
            command = Enumerations_1.Enumerations.CHANNEL_MODE_MESSAGES[command];
        if (WebMidi_1.WebMidi.validation) {
    ...
    sendChannelAftertouch(pressure, options) {
        ...
        this.send([
            (Enumerations_1.Enumerations.CHANNEL_MESSAGES.channelaftertouch << 4) + (this.number - 1),
    ...
    sendPitchBend(value, options) {
        ...
        if (options.rawValue && Array.isArray(value)) {
            ...
            const result = Utilities_1.Utilities.fromFloatToMsbLsb((value + 1) / 2); // b/c value is -1 to 1
            msb = result.msb;
            lsb = result.lsb;
        }
        this.send([
            (Enumerations_1.Enumerations.CHANNEL_MESSAGES.pitchbend << 4) + (this.number - 1),
            lsb,
            msb
        ], { time: Utilities_1.Utilities.toTimestamp(options.time) });
        return this;
    ...
    sendProgramChange(program, options) {
        ...
        this.send([
            (Enumerations_1.Enumerations.CHANNEL_MESSAGES.programchange << 4) + (this.number - 1),
            program
        ], { time: Utilities_1.Utilities.toTimestamp(options.time) });
        return this;
    }
    ...
    sendRpnValue(rpn, data, options) {
        if (!Array.isArray(rpn))
            rpn = Enumerations_1.Enumerations.REGISTERED_PARAMETERS[rpn];
        if (WebMidi_1.WebMidi.validation) {
