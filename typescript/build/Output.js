 ...
 * @license Apache-2.0
 */
class Output extends EventEmitter_1.EventEmitter {
    /**
    ...
    constructor(midiOutput) {
        ...
        this.channels = [];
        for (let i = 1; i <= 16; i++)
            this.channels[i] = new OutputChannel_1.OutputChannel(this, i);
        this._midiOutput.onstatechange = this._onStateChange.bind(this);
    }
    _onStateChange(e) {
        let event = {
            timestamp: WebMidi_1.WebMidi.time
        };
    ...
    playNote(note, options) {
        ...
        if (options.channels == undefined)
            options.channels = Enumerations_1.Enumerations.MIDI_CHANNEL_NUMBERS;
        Utilities_js_1.Utilities.sanitizeChannels(options.channels).forEach(ch => {
    ...
    send(message, options, legacy) {
        ...
        if (message instanceof Message_1.Message) {
            message = Utilities_js_1.Utilities.isNode ? message.data : message.rawData;
        }
        // If the data is a Uint8Array and we are on Node, we must convert it to array so it works with
        // the jzz module.
        if (message instanceof Uint8Array && Utilities_js_1.Utilities.isNode) {
        ...
        this._midiOutput.send(message, Utilities_js_1.Utilities.toTimestamp(options.time));
        return this;
    }
    /**
    ...
    sendSysex(identification, data, options) {
        ...
        if (data instanceof Uint8Array) {
            ...
            merged[0] = Enumerations_1.Enumerations.SYSTEM_MESSAGES.sysex;
            merged.set(Uint8Array.from(identification), 1);
            merged.set(data, 1 + identification.length);
            merged[merged.length - 1] = Enumerations_1.Enumerations.SYSTEM_MESSAGES.sysexend;
            this.send(merged, { time: options.time });
        }
        else {
            const merged = identification.concat(data, Enumerations_1.Enumerations.SYSTEM_MESSAGES.sysexend);
            this.send([Enumerations_1.Enumerations.SYSTEM_MESSAGES.sysex].concat(merged), { time: options.time });
        }
        return this;
    }
    ;
    ...
    sendTimecodeQuarterFrame(value, options) {
        ...
        this.send([
            Enumerations_1.Enumerations.SYSTEM_MESSAGES.timecode,
            value
        ], { time: options.time });
        return this;
    }
    ...
    sendSongPosition(value, options) {
        ...
        var lsb = value & 0x7F;
        this.send([
            Enumerations_1.Enumerations.SYSTEM_MESSAGES.songposition,
            msb,
            lsb
        ], { time: options.time });
        return this;
    ...
    sendSongSelect(value, options) {
        ...
        this.send([
            Enumerations_1.Enumerations.SYSTEM_MESSAGES.songselect,
            value
        ], { time: options.time });
        return this;
    }
    ...
    sendTuneRequest(options) {
        this.send([Enumerations_1.Enumerations.SYSTEM_MESSAGES.tunerequest], { time: options.time });
        return this;
    }
    /**
     ...
     */
    sendClock(options) {
        this.send([Enumerations_1.Enumerations.SYSTEM_MESSAGES.clock], { time: options.time });
        return this;
    }
    /**
     ...
     */
    sendStart(options) {
        this.send([Enumerations_1.Enumerations.SYSTEM_MESSAGES.start], { time: options.time });
        return this;
    }
    /**
    ...
    sendContinue(options) {
        this.send([Enumerations_1.Enumerations.SYSTEM_MESSAGES.continue], { time: options.time });
        return this;
    }
    /**
     ...
     */
    sendStop(options) {
        this.send([Enumerations_1.Enumerations.SYSTEM_MESSAGES.stop], { time: options.time });
        return this;
    }
    /**
    ...
    sendActiveSensing(options) {
        this.send([Enumerations_1.Enumerations.SYSTEM_MESSAGES.activesensing], { time: options.time });
        return this;
    }
    /**
     ...
     */
    sendReset(options) {
        this.send([Enumerations_1.Enumerations.SYSTEM_MESSAGES.reset], { time: options.time });
        return this;
    }
    /**
