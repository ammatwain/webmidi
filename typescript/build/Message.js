...
class Message {
    ...
    constructor(data) {
        ...
        if (this.isChannelMessage) {
            this.type = Utilities_1.Utilities.getPropertyByValue(Enumerations_1.Enumerations.CHANNEL_MESSAGES, this.command);
        }
        else if (this.isSystemMessage) {
            this.type = Utilities_1.Utilities.getPropertyByValue(Enumerations_1.Enumerations.SYSTEM_MESSAGES, this.command);
        }
        // When the message is a sysex message, we add a manufacturer property and strip out the id from
        // dataBytes and rawDataBytes.
        if (this.statusByte === Enumerations_1.Enumerations.SYSTEM_MESSAGES.sysex) {
