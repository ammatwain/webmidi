 ...
 * @since 3.0.0
 */
class InputChannel extends EventEmitter_1.EventEmitter {
    /**
    ...
    _dispatchParameterNumberEvent(type, paramMsb, paramLsb, e) {
        ...
        const event = {
            ...
            value: Utilities_1.Utilities.from7bitToFloat(e.message.dataBytes[1]),
            rawValue: e.message.dataBytes[1],
        };
        // Identify the parameter (by name for RPN and by number for NRPN)
        if (type === "rpn") {
            event.parameter = Object.keys(Enumerations_1.Enumerations.REGISTERED_PARAMETERS).find(key => {
                return Enumerations_1.Enumerations.REGISTERED_PARAMETERS[key][0] === paramMsb &&
                    Enumerations_1.Enumerations.REGISTERED_PARAMETERS[key][1] === paramLsb;
            });
        }
        else {
        ...
        // Type and subtype
        const subtype = Enumerations_1.Enumerations.CONTROL_CHANGE_MESSAGES[e.message.dataBytes[0]].name;
        // Emit specific event
    ...
    _parseEventForStandardMessages(e) {
        ...
        if (event.type === "noteoff" || (event.type === "noteon" && data2 === 0)) {
            ...
            event.note = new Note_1.Note(Utilities_1.Utilities.offsetNumber(data1, this.octaveOffset + this.input.octaveOffset + WebMidi_1.WebMidi.octaveOffset), {
                rawAttack: 0,
                rawRelease: data2,
            });
            event.value = Utilities_1.Utilities.from7bitToFloat(data2);
             ...
             * and 127).
             */
            event.note = new Note_1.Note(Utilities_1.Utilities.offsetNumber(data1, this.octaveOffset + this.input.octaveOffset + WebMidi_1.WebMidi.octaveOffset), { rawAttack: data2 });
            event.value = Utilities_1.Utilities.from7bitToFloat(data2);
            event.rawValue = data2;
            // Those are kept for backwards-compatibility but are gone from the documentation. They will
            // be removed in future versions (@deprecated).
            ...
            event.controller = {
                number: data1,
                name: Enumerations_1.Enumerations.CONTROL_CHANGE_MESSAGES[data1].name,
                description: Enumerations_1.Enumerations.CONTROL_CHANGE_MESSAGES[data1].description,
                position: Enumerations_1.Enumerations.CONTROL_CHANGE_MESSAGES[data1].position,
            };
            ...
            namedEvent.type = `${event.type}-` + Enumerations_1.Enumerations.CONTROL_CHANGE_MESSAGES[data1].name;
            ...
            if (this.parameterNumberEventsEnabled &&
             ...
             * @property {number} rawValue The raw MIDI value expressed as an integer between 0 and 127.
             */
            event.value = Utilities_1.Utilities.from7bitToFloat(data1);
