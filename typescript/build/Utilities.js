...
class Utilities {
    ...
    static buildNote(input, options) {
        ...
        if (input instanceof Note_1.Note)
            return input;
        ...
        return new Note_1.Note(number, options);
    }
    /**
     ...
     */
    static fromMsbLsbToFloat(msb, lsb) {
        if (WebMidi_1.WebMidi.validation) {
    ...
    static getCcNameByNumber(number) {
        ...
        return Enumerations_1.Enumerations.CONTROL_CHANGE_MESSAGES[number].name;
    }
    /**
    ...
    static getCcNumberByName(name) {
        let message = Enumerations_1.Enumerations.CONTROL_CHANGE_MESSAGES.find(element => element.name === name);
        if (message) {
            return message.number;
        }
        else {
            // Legacy (remove in v4)
            return Enumerations_1.Enumerations.MIDI_CONTROL_CHANGE_MESSAGES[name];
        }
    }
    /**
    ...
    static getChannelModeByNumber(number) {
        if (!(number >= 120 && number <= 127))
            return false;
        for (let cm in Enumerations_1.Enumerations.CHANNEL_MODE_MESSAGES) {
            if (Enumerations_1.Enumerations.CHANNEL_MODE_MESSAGES.hasOwnProperty(cm) &&
                number === Enumerations_1.Enumerations.CHANNEL_MODE_MESSAGES[cm]) {
                return cm;
            }
        }
        return false;
     ...
     * @static
     */
    static offsetNumber(number, octaveOffset, semitoneOffset) {
        if (WebMidi_1.WebMidi.validation) {
            number = parseInt(number);
            if (isNaN(number))
    ...
    static sanitizeChannels(channel) {
        ...
        if (channels.indexOf("all") > -1) {
            channels = Enumerations_1.Enumerations.MIDI_CHANNEL_NUMBERS;
        }
        return channels
            .map(function (ch) {
            return parseInt(ch);
     ...
     * @static
     */
    static toNoteNumber(identifier, octaveOffset) {
        // Validation
    ...
    static toTimestamp(time) {
        ...
        if (typeof time === "string" && time.substring(0, 1) === "+") {
            if (parsed >= 0)
                value = WebMidi_1.WebMidi.time + parsed;
        }
        else {
            if (parsed >= 0)
                value = parsed;
