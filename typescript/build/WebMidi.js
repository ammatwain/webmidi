...
// isn't needed.
if (Utilities_1.Utilities.isNode) {
 ...
 * @license Apache-2.0
 */
class WebMidi extends EventEmitter_1.EventEmitter {
    /**
    ...
    _updateInputs() {
        ...
        this.interface.inputs.forEach(nInput => {
            ...
            if (!this._inputs.find(input => input._midiInput === nInput)) {
                ...
                if (!input)
                    input = new Input_1.Input(nInput);
    ...
    _updateOutputs() {
        ...
        this.interface.outputs.forEach(nOutput => {
            ...
            if (!this._outputs.find(output => output._midiOutput === nOutput)) {
                ...
                if (!output)
                    output = new Output_1.Output(nOutput);
    ...
    /* async */ enable(options) {
        ...
        try {
            if (typeof options.requestMIDIAccessFunction === "function") {
                this.interface = await options.requestMIDIAccessFunction({ sysex: options.sysex, software: options.software });
            }
            else {
                this.interface = await navigator.requestMIDIAccess({ sysex: options.sysex, software: options.software });
            }
        }
        catch (err) {
